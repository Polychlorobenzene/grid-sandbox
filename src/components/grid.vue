<template>
  <div class="dataGrid">    
    <div v-if="pageCount" id="gridNavTop">
      ...
      <a :disabled="isFirstPage" class="btn" href id="firstPage" @click.prevent="getPageResults(1)">
      ...
      <span class="page-span" id="page-span">
        <span :id="'page-span-'+ item.page" v-for="(item, index) in visiblePages" :key="item.page">
          <a
            href
      ...
      <input type="text" id="filter" v-model="filterTextBuffer" @keyup="delayFilter" />
...
export default class DataGrid extends Vue {
  ...
  get displayData(): any[] {
    ...
    const startIndex: number = this.currentPage * this.displayRecordCount;
